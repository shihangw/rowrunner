...
import { ProgressSampleSchema, SceneOptionsSchema, WorldPluginSchema } from 'rowrunner/schemas';
...
const sample: ProgressSample = { runId: 'migration', completed: 500, timestamp: Date.now() };
const sink = new ProgressSink();
const result = sink.push(sample);
if (result.accepted) result.sample.completed.toFixed();
