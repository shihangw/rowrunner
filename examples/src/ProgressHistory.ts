...
export class ProgressHistory {
  ...
  add(result: PushResult) {
    ...
    if (result.reason === 'new-run' || result.reason === 'counter-reset') this.reset();
    ...
    if (this.points.length > 2048)
      this.points = this.points.filter((_, i, all) => i % 2 === 0 || i === all.length - 1);
    this.version++;
  }
  plot() {
    if (!this.points.length) return { line: '', area: '', last: null, min: 0, max: 1, duration: 0 };

    const latestSample = this.points.at(-1)!;
    const windowStartTimestamp = Math.max(this.points[0].at, latestSample.at - 60_000);
    const firstVisibleIndex = this.points.findIndex((sample) => sample.at >= windowStartTimestamp);
    ...
    if (firstVisibleIndex > 0 && visibleSamples[0].at > windowStartTimestamp) {
      ...
      const boundaryFraction =
        (windowStartTimestamp - previousSample.at) / (nextSample.at - previousSample.at);
    ...
    const minimumCount = Math.min(...visibleSamples.map((sample) => sample.completed));
    const maximumCount = Math.max(...visibleSamples.map((sample) => sample.completed));
    const recentGrowthRate =
      duration > 0
        ? Math.max(0, (latestSample.completed - firstSample.completed) / (duration / 1000))
        : 0;
    ...
    const lowerTickInterval = Math.max(1, 10 ** Math.floor(Math.log10(lowerPadding)));
    const min = Math.max(
      0,
      Math.floor((minimumCount - lowerPadding) / lowerTickInterval) * lowerTickInterval,
    );
    const max = Math.max(
      min + tickInterval,
