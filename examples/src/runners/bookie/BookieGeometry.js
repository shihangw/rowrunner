...
import { drawExhaust } from '../shared/ExhaustParticles.js';
...
export function drawBookie(geometryBuilder, time, rate, isMotionReduced) {
  ...
  for (const y of [0.32, 0.43, 0.54])
    geometryBuilder.box(0.15, y, -1.035, 2.52, 0.018, 0.018, [0.55, 0.59, 0.61]);
  // Body and round charcoal head.
  geometryBuilder.ellipsoid([0, 1.2, 0.1], [0.67, 0.59, 0.58], [0.23, 0.28, 0.31], 40, 28, true);
  geometryBuilder.ellipsoid([0, 2.22, 0], [1.13, 1.13, 1.13], [0.32, 0.37, 0.4], 64, 40, true);
  ...
  for (const side of [-1, 1]) {
    ...
    geometryBuilder.ellipsoid([points[3][0], 3.82, -0.035], [0.14, 0.14, 0.12], gold);
  ...
  for (const side of [-1, 1]) {
    const x = side * 0.49;
    geometryBuilder.ellipsoid(facePoint([side * 0.68, 1.91, -0.62]), [0.18, 0.085, 0.045], gold);
    geometryBuilder.ellipsoid(facePoint([x, 2.31, -0.74]), [0.4, 0.4, 0.15], cream, 24, 12);
    geometryBuilder.ellipsoid(
  ...
  for (let i = 1; i < smile.length; i++)
    geometryBuilder.tube(facePoint(smile[i - 1]), facePoint(smile[i]), 0.026, ink);
  for (const side of [-1, 1]) {
    geometryBuilder.ellipsoid([side * 0.29, 1.1, -0.6], [0.31, 0.23, 0.14], ink);
  ...
  for (const side of [-1, 1])
    drawExhaust(geometryBuilder, time, rate, isMotionReduced, [side * 0.97, 0.15, 1.1], 1);
}
