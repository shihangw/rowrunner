export function drawPrism(geometryBuilder, time, rate, isMotionReduced) {
  ...
  const rotate = ([x, y, z], angle) => {
    const rotationCosine = Math.cos(angle),
      rotationSine = Math.sin(angle),
      tilt = 0.38;
    const rotatedX = x * rotationCosine - z * rotationSine,
      rotatedZ = x * rotationSine + z * rotationCosine;
    return [
      rotatedX,
  ...
  for (let i = 0; i < faces.length; i++) {
    const face = faces[i];
    geometryBuilder.glassTriangle(...face.map((v) => vertices[v]), colors[i % colors.length], 0.76);
    for (let j = 0; j < 3; j++) {
      const a = face[j],
        b = face[(j + 1) % 3],
  ...
  const breath = isMotionReduced ? 0.5 : (1 - Math.cos((time * Math.PI * 2) / 4.5)) / 2;
  geometryBuilder.particle([0, 1.5, 0], 3.1 + breath * 0.6, [0.3, 0.68, 1], 0.32 + breath * 0.18);
