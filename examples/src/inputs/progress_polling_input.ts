...
export function startProgressPolling(
  ...
  const {intervalMilliseconds, timeoutMilliseconds} =
    PollingTimingSchema.parse(options);
  let isStopped = false;
  ...
  async function poll() {
    ...
    try {
      const sample = ProgressSampleSchema.parse(
        await options.readSample(request.signal),
      );
