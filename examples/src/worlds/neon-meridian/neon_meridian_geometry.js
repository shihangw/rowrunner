import {random} from '../shared/deterministic_random.js';
export function drawNeon(
  ...
  const blocksView = (x, z, w, h, depth) => {
    ...
    const high = [x + w / 2, h + 25, z + depth / 2];
    let enter = 0;
    let exit = 1;
    for (let axis = 0; axis < 3; axis++) {
      ...
      if (Math.abs(delta) < 1e-8) {
        if (view.eye[axis] < low[axis] || view.eye[axis] > high[axis]) {
          return false;
        }
      } else {
        const a = (low[axis] - view.eye[axis]) / delta;
        const b = (high[axis] - view.eye[axis]) / delta;
  ...
  const violet = [0.65, 0.3, 0.88];
  const amber = [0.95, 0.57, 0.25];
  ...
  for (
    ...
    for (const side of [-1, 1]) {
      ...
      const x = roadCenterXAt(z) + side * (21 + random(seed) * 5);
      const h = 3 + random(seed + 1) * 6;
      const light = random(seed + 2) > 0.5 ? cyan : violet;
      if (blocksView(x, z, 14, h, 24)) {
        continue;
      }
  ...
  for (let layer = 0; layer < districts.length; layer++) {
    ...
    for (
      ...
      for (const side of [-1, 1]) {
        ...
        const back = z + depth / 2 + 0.025;
        const stride = layer === 2 ? 10 : 5;
        for (let level = 3; level < h - 2; level += stride) {
          const color = light.map(
            (v) => v * (0.35 + random(seed + level) * 0.4),
          );
          const y = level;
          const top = y + 0.28;
          geometryBuilder.quad(
