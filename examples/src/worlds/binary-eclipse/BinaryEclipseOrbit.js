...
export function orbitPoint([x, y, z], distance) {
  const angle = (((distance + z) % LOCAL_ORBIT_LENGTH) * ORBIT_TRAVEL_SCALE) / ORBIT_RADIUS;
...
export function createBinaryOrbit(distance, entryDistance = 0) {
  ...
  return { center: BINARY_CENTER, radius: ORBIT_RADIUS, origin, worldPoint, localPoint };
}
...
export function orbitCamera(pose, stationary, completed) {
  if (stationary || completed) return pose;
  return {
    ...pose,
