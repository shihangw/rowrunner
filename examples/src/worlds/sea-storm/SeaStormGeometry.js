...
export function stormFlash(time, reduced = false) {
  ...
  return phase > 8 && phase < 9.6 ? Math.sin(((phase - 8) / 1.6) * Math.PI) ** 2 : 0;
}
...
const WAVES = [
  ...
  const k = (2 * Math.PI) / length,
    n = Math.hypot(x, z);
  return { amplitude, kx: (k * x) / n, kz: (k * z) / n, speed: Math.sqrt(9.81 * k), phase };
});
...
function waterTriangle(m, a, b, c) {
  if (m.gradientTriangle) m.gradientTriangle(a.point, b.point, c.point, a.color, b.color, c.color);
  else if (m._gradientTriangle)
    m._gradientTriangle(a.point, b.point, c.point, a.color, b.color, c.color);
  else m.triangle(a.point, b.point, c.point, a.color, true);
}
export function drawStorm(
  ...
  const sample = (x, wz) => {
    ...
    const glint =
      Math.pow(Math.max(0, 1 - Math.abs(dx - 0.075) * 6 - Math.abs(dz + 0.025) * 5), 8) * 0.1;
    ...
    const foam =
      high * high * (3 - 2 * high) * Math.max(0, Math.min(1, (patch + 0.1) * 1.6)) * 0.72;
    const base = [
  ...
  for (let wz = start; wz < end; wz += 6) {
    const next = Array.from({ length: 49 }, (_, i) => sample((i - 24) * 6, wz + 6));
    for (let i = 0; i < 48; i++) {
  ...
  for (const side of [-1, 1]) {
    let row = Array.from({ length: 27 }, (_, i) => sample(side * (144 + i * 18), start));
    for (let wz = start; wz < end; wz += 18) {
      const next = Array.from({ length: 27 }, (_, i) => sample(side * (144 + i * 18), wz + 18));
  ...
  for (
    ...
    geometryBuilder.box(roadCenterXAt(z), -9, z, 10.8, 8.2, 1.2, [0.055, 0.09, 0.12]);
    for (const side of [-1, 1]) {
      const x = roadCenterXAt(z) + side * (18 + random(i + side) * 14),
        y = seaHeight(x, i * 56, time);
      geometryBuilder.box(x, y - 0.3, z, 1.1, 2.3, 1.1, [0.14, 0.19, 0.21]);
      geometryBuilder.box(x, y + 1.8, z, 1.12, 0.3, 1.12, [0.95, 0.52, 0.19]);
      geometryBuilder.ellipsoid([x, y + 2.4, z], [0.2, 0.24, 0.2], [1, 0.69, 0.3], 8, 5);
      for (let rock = 0; rock < 2; rock++) {
        const rz = z + rock * 19,
          rx = roadCenterXAt(rz) + side * (58 + rock * 68 + random(i + rock) * 26);
  ...
  for (
    ...
    for (const side of [-1, 1]) {
      const x = roadCenterXAt(z) + side * (80 + random(i + side) * 95),
        height = 24 + random(i * 3 + side) * 22;
      for (const leg of [-1, 1])
        geometryBuilder.box(x + leg * 5, -8, z, 1.1, 12, 1.1, [0.09, 0.14, 0.17]);
      geometryBuilder.box(x, 3, z, 15, 1.1, 11, [0.11, 0.18, 0.21]);
      geometryBuilder.box(x, 4.1, z, 1.0, height, 1.0, [0.22, 0.29, 0.31]);
      geometryBuilder.ellipsoid([x, height + 4, z], [1.0, 1.0, 0.6], [0.37, 0.47, 0.49], 8, 5);
  ...
  for (
    ...
    for (let drop = 0; drop < 32; drop++) {
      ...
      const y = 104 - ((time * (18 + random(seed) * 12) + random(seed + 3) * 110) % 110);
      if (y < -2 || Math.abs(x - roadCenterXAt(z)) < 6) continue;
