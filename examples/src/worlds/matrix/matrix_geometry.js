...
export function drawMatrix(
  ...
  for (
    ...
    for (const side of [-1, 1]) {
      ...
      const x = roadCenterXAt(z) + side * (27 + random(seed) * 13);
      ...
      for (let column = 0; column < 3; column++) {
        ...
        const head =
          height - ((time * speed + random(streamSeed + 11) * cycle) % cycle);
        for (let row = 0; row < 12; row++) {
          ...
          const index = Math.floor(
            random(streamSeed * 13 + row) * GLYPHS.length,
          );
          const color =
            row === 0
              ? [0.65, 1, 0.74]
              : palette.accent.map((v) => v * (0.85 - row * 0.058));
          glyph(
            geometryBuilder,
            x - 3.4 + column * 2.4,
            y,
      ...
      const rackX =
        roadCenterXAt(z + 28) + side * (65 + random(seed + 21) * 30);
      const rackZ = z + 28;
      for (let rack = 0; rack < 3; rack++) {
        const xx = rackX + side * rack * 6;
        const hh = 5 + random(seed + rack + 40) * 9;
      ...
      const nodeZ = z + 38;
      const nodeX = roadCenterXAt(nodeZ) + side * 17;
      const endX = roadCenterXAt(nodeZ) + side * 112;
      geometryBuilder.quad(
        [nodeX, -0.94, nodeZ],
        [endX, -0.94, nodeZ],
      ...
      const streamX =
        roadCenterXAt(z + 20) + side * (12 + random(seed + 2) * 5);
      ...
      for (let row = 0; row < 10; row++) {
        ...
        glyph(
          ...
          row === 0
            ? [0.72, 1, 0.8]
            : palette.accent.map((v) => v * (0.8 - row * 0.065)),
        );
      }
    }
  }
  ...
  for (
    ...
    for (const side of [-1, 1]) {
      for (let layer = 0; layer < 2; layer++) {
        ...
        const zz = z + layer * 45;
        const x =
          roadCenterXAt(zz) + side * (245 + layer * 95 + random(seed) * 35);
        const height = 65 + random(seed + 5) * 95;
        const width = 14 + random(seed + 8) * 13;
        geometryBuilder.box(
          x,
          -1,
          zz,
  ...
  for (
    ...
    for (const side of [-1, 1]) {
      for (let layer = 0; layer < 3; layer++) {
        ...
        const x =
          roadCenterXAt(zz) + side * (54 + layer * 66 + random(seed + 4) * 18);
        const top = 55 + layer * 28;
        const cycle = top + 25;
        const head =
          top -
          ((time * (4 + random(seed + 3) * 4) + random(seed + 9) * cycle) %
            cycle);
        ...
        for (let row = 0; row < rows; row++) {
          ...
          const color =
            row === 0
              ? [0.55, 0.95, 0.69]
              : palette.accent.map((v) => v * (0.5 - row * 0.045));
          glyph(
            geometryBuilder,
            x,
            y,
