import {random} from './deterministic_random.js';
import {platforms} from './platform_geometry.js';
  ...
  for (let layer = 0; layer < bands.length; layer++) {
    ...
    for (const side of [-1, 1]) {
      const section = (index) => {
        ...
        const seed = index * 37 + layer * 19 + side * 11;
      ...
      const end = Math.ceil((d + far + step) / step);
      let a = section(start);
      for (let i = start; i < end; i++) {
        ...
        for (let strip = 0; strip < 4; strip++) {
          for (const points of [
            ...
            const shade =
              0.82 + random(i * 13 + strip * 7 + side + layer) * 0.22;
            const snow =
              ice && layer > 0 ? Math.max(0, (summit - 0.58) / 0.42) * 0.62 : 0;
            const color = colors[layer].map((v, axis) =>
              Math.min(
                1,
                v * shade * (1 - snow) + [0.68, 0.84, 0.87][axis] * snow,
              ),
            );
...
export function drawTerrain(
  ...
  for (
    ...
    if (i % 6 === 0) {
      platforms(geometryBuilder, roadCenterXAt, z, 1, palette);
    }
  }
}
