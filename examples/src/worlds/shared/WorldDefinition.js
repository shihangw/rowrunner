...
import { worldSky } from './SkyShaderUtilities.js';
export function defineWorld({ ground = false, guideLights = true, sky = 0, draw, ...definition }) {
  return define({
    ...definition,
    sky: worldSky(sky),
    draw(geometryBuilder, sceneFrame) {
      ...
      draw(
        Object.create(geometryBuilder, {
          model: { value: (name, ...args) => geometryBuilder.model(SPACE_MODELS[name], ...args) },
        }),
        sceneFrame,
      );
      if (guideLights)
