...
import {platforms} from '../shared/platform_geometry.js';
export function drawHarbor(
  ...
  for (
    ...
    for (const side of [-1, 1]) {
      ...
      const tip = [x, -42, z];
      const a = [x - width, -9, z - width];
      const b = [x + width, -9, z - width];
      const c = [x + width, -9, z + width];
      const e = [x - width, -9, z + width];
      for (const [v, w] of [
        [a, b],
        [b, c],
        [c, e],
      ...
      if (i % 3 === 0) {
        platforms(geometryBuilder, roadCenterXAt, z, side, palette);
      }
    }
    if (((i % 3) + 3) % 3 === 1) {
      geometryBuilder.model(
