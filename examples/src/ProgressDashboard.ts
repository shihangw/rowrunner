...
import { worlds as exampleWorlds, runners as exampleRunners } from './ExampleConfiguration.ts';
...
import { ProgressHistory } from './ProgressHistory.ts';
import { startProgressPolling } from './inputs/ProgressPollingInput.ts';

export function startDashboard(
  sceneOptions: SceneOptions,
  celebrationOptions: { message: string },
) {
  const elementWithID = <T extends HTMLElement = HTMLElement>(id: string): T => {
  ...
  let selectedWorldID: string, selectedRunnerID: string;
  try {
    roadScene = new RoadScene(elementWithID<HTMLCanvasElement>('road'), sceneOptions);
  } catch (error) {
  ...
  const worlds =
    roadScene?.scenes ??
    exampleWorlds.map((p) => ({ ...p, name: p.name ?? p.id, description: p.description ?? '' }));
  const availableRunners =
    roadScene?.protagonists ??
    exampleRunners.map((p) => ({ ...p, name: p.name ?? p.id, description: p.description ?? '' }));
  ...
  function updateContentSelection() {
    ...
    document.querySelector<HTMLElement>('.viewport')!.dataset.world = selectedWorldID;
    for (const button of document.querySelectorAll<HTMLElement>('[data-scene], [data-mascot]')) {
  ...
  for (const [kind, presets] of [
    ...
    for (const preset of presets) {
      ...
      icon.textContent =
        ({ bookie: '◎', courier: '◇', 'paper-plane': '➤' } as Record<string, string>)[preset.id] ??
        '';
  ...
  elementWithID('auto-biomes').setAttribute('aria-pressed', String(roadScene?.autoBiomes ?? false));
  ...
  elementWithID<HTMLSelectElement>('road-effect').onchange = () =>
    roadScene?.setRoadEffect(
      RoadEffectSchema.parse(elementWithID<HTMLSelectElement>('road-effect').value),
    );
  elementWithID<HTMLSelectElement>('camera-mode').onchange = () =>
    roadScene?.setCamera(
      CameraModeSchema.parse(elementWithID<HTMLSelectElement>('camera-mode').value),
    );
  elementWithID('auto-biomes').onclick = () => {
    const enabled = elementWithID('auto-biomes').getAttribute('aria-pressed') !== 'true';
    roadScene?.setAutoBiomes(enabled);
    elementWithID('auto-biomes').setAttribute('aria-pressed', String(enabled));
    elementWithID('auto-biomes').textContent = enabled ? 'Auto journey on' : 'Hold this biome';
  };
      ...
      acceptSample(
        {
          ...
          status: isDemoCompleted ? 'completed' : isDemoPaused ? 'paused' : 'running',
  ...
  setInterval(() => {
    if (inputMode === 'demo') publishDemoProgress(Date.now());
  }, 2000);
  ...
  function setProgressInputMode(next: 'demo' | 'live') {
    ...
    const isSolanaInput =
      next === 'live' && elementWithID<HTMLSelectElement>('live-source').value === 'solana';
    ...
    elementWithID('progress-heading').textContent = isSolanaInput ? 'NETWORK TOTAL' : 'COMPLETED';
    ...
    if (next === 'demo') {
    ...
    } else if (!isSolanaInput) connectToProgressEvents();
    else {
      ...
      stopProgressPolling = startProgressPolling({
        ...
        async readSample(signal) {
          ...
          if (!response.ok) throw new Error(`Progress request returned HTTP ${response.status}`);
          return ProgressSampleSchema.parse(await response.json());
        },
        onSample(sample) {
          acceptSample(sample);
          elementWithID('solana-connection').textContent = 'Connected · polling every 2s';
        },
        onError() {
  ...
  function connectToProgressEvents() {
    if (!elementWithID<HTMLFormElement>('connect-form').reportValidity()) return;
    progressEventSource?.close();
    resetProgressDisplay();
    ...
    progressEventSource = new EventSource(`/api/runs/${encodeURIComponent(runId)}/events`);
    ...
    progressEventSource.onmessage = (event) => {
      try {
        ...
        elementWithID('announcement').textContent = `Invalid sample: ${String(error)}`;
      }
    };
  }
  ...
  function updateDemoControls() {
    ...
    elementWithID('signal').textContent = isSignalLost ? 'Restore signal' : 'Lose signal';
    elementWithID('signal').classList.toggle('active', isSignalLost);
    elementWithID('finish').textContent = isDemoCompleted ? 'New run ↗' : 'Finish ↗';
  }
  ...
  elementWithID('signal').onclick = () => {
    ...
    updateDemoControls();
    if (!isSignalLost) publishDemoProgress(Date.now());
  };
  ...
  function updateProgressDashboard(snapshot: ProgressSnapshot, currentTimestamp: number) {
    elementWithID('state-badge').textContent = progressStateLabels[snapshot.state];
    ...
    elementWithID('completed').textContent = countFormatter.format(snapshot.completed);
    ...
    if (snapshot.progress === null) track.removeAttribute('aria-valuenow');
    else track.setAttribute('aria-valuenow', String(snapshot.progress * 100));
    elementWithID('percent').textContent =
      snapshot.progress === null ? 'Open-ended' : `${(snapshot.progress * 100).toFixed(1)}%`;
    elementWithID('eta').textContent = formattedTimeRemaining(snapshot.etaSeconds);
    ...
    if (renderedChartVersion !== progressHistory.version) {
      ...
      elementWithID('chart-max').textContent = chart.last ? countFormatter.format(chart.max) : '—';
      elementWithID('chart-min').textContent = chart.last ? countFormatter.format(chart.min) : '—';
    ...
    if (snapshot.state !== previousProgressState) {
      elementWithID('announcement').textContent = progressStateLabels[snapshot.state];
  ...
  function updateSpeedometer(
    ...
    const strength = snapshot.state === 'running' ? Math.min(1, rateSmoother.value / 120) : 0;
    needleFlutterStrength +=
      (strength - needleFlutterStrength) * (1 - Math.exp(-deltaSeconds / 0.25));
    if (gaugeReducedMotion.matches) needleFlutterStrength = 0;
    ...
    const flutter =
      wave * needleFlutterStrength * Math.min(rateSmoother.value * 0.03, (2000 * 1.2) / 180);
    ...
    if (elementWithID('speed').textContent !== text) elementWithID('speed').textContent = text;
    ...
    elementWithID('gauge-scale').textContent = displayRate > 2000 ? '2k+' : '2k';
  }
  let previousFrameTime = performance.now();
  function renderAnimationFrame(animationTimeMilliseconds: number) {
    const deltaSeconds = Math.max(0, (animationTimeMilliseconds - previousFrameTime) / 1000);
    previousFrameTime = animationTimeMilliseconds;
    const currentTimestamp = Date.now(),
      snapshot = progressSink.snapshot(currentTimestamp);
    ...
    roadScene?.render(rateSmoother.value, deltaSeconds, { completed: isCompleted });
    ...
    completionCelebration.update(snapshot, { source: inputMode, now: animationTimeMilliseconds });
    if (animationTimeMilliseconds - lastDashboardUpdateTime > 100) {
      updateProgressDashboard(snapshot, currentTimestamp);
