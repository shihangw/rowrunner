import {RoadEffectSchema} from './scene_configuration_schemas.js';
import type {
  Color,
  Vec3,
  Palette,
...
export function roadSurface(
  ...
  return {
    amount,
    road: isRenewalEnabled
      ? mix(dormantRoadColor, polishedRoadColor, amount)
      : palette.road,
    accent: isRenewalEnabled
      ? mix([0.055, 0.065, 0.075], palette.accent, amount)
      : palette.accent,
  };
}
...
export function drawRoad(
  ...
  for (
    ...
    if (isRenewalEnabled) {
      ...
      drawRoadStrip(
        ...
        mix(
          [0.027, 0.031, 0.038],
          scale(palette.accent, 0.28),
          surfaceColors.amount,
        ),
        true,
      );
      for (const side of [-1, 1]) {
  ...
  for (
    ...
    const surfaceColors = surfaceColorsAt(z);
