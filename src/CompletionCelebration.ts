import type { ProgressSnapshot } from './ProgressDataTypes.js';
interface Particle {
  side: number;
  delay: number;
  speed: number;
...
export class CompletionCelebration {
  ...
  constructor(container: HTMLElement, { message = 'Congrats!' } = {}) {
    if (typeof message !== 'string') throw new TypeError('Completion message must be a string');
    ...
    this.startedAt = null;
    this.particles = [];
    this.motion = doc.defaultView!.matchMedia('(prefers-reduced-motion: reduce)');
  ...
  update(
    ...
    const key = snapshot.runId ? JSON.stringify([source, snapshot.runId]) : null;
    if (snapshot.state !== 'completed' || !key) {
      if (this.activeKey !== null) this.reset();
      return;
    }
  ...
  draw(elapsed: number) {
    ...
    const ratio = Math.min(this.element.ownerDocument.defaultView!.devicePixelRatio || 1, 2);
    ...
    for (const p of this.particles) {
      ...
      const x = (p.side < 0 ? 0.015 + p.speed * flight : 0.985 - p.speed * flight) * width;
      ...
      ctx.fillRect(-p.size / 2, -p.size / 2, p.size, p.streamer ? p.size * 3.5 : p.size * 0.65);
      ctx.restore();
    }
