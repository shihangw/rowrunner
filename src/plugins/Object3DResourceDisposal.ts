import type { Object3D, BufferGeometry, Material, Skeleton, InstancedMesh, Texture } from 'three';
...
export function disposeObject3D(object: Object3D): void {
  ...
  const texture = (value: unknown): value is Texture =>
    !!value && typeof value === 'object' && 'isTexture' in value && value.isTexture === true;
  object.traverse((child) => {
    ...
    if (node.isInstancedMesh) resources.add(node as InstancedMesh);
    if (node.geometry) resources.add(node.geometry);
    if (node.skeleton) resources.add(node.skeleton);
    for (const material of Array.isArray(node.material) ? node.material : [node.material]) {
      if (!material) continue;
      resources.add(material);
      for (const value of Object.values(material)) if (texture(value)) resources.add(value);
      ...
      for (const uniform of Object.values(uniforms ?? {})) {
        for (const value of Array.isArray(uniform.value) ? uniform.value : [uniform.value])
          if (texture(value)) resources.add(value);
      }
    }
  });
