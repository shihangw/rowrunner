import type { WorldPlugin, RunnerPlugin } from '../SceneTypes.js';
type Plugin = WorldPlugin | RunnerPlugin;
import { defineWorld, defineRunner } from './PluginDefinitions.js';
...
export function createPluginRegistry(plugins: readonly Plugin[] = []) {
  ...
  const inputs = { world: new Map<string, Plugin>(), runner: new Map<string, Plugin>() };
  for (const input of plugins as readonly Plugin[]) {
    if (!input || !['world', 'runner'].includes(input.kind))
      throw new TypeError('Use defineWorld() or defineRunner() to declare a plugin');
  ...
  function resolveAll(kind: 'world' | 'runner'): readonly Plugin[] {
    ...
    function resolve(id: string): Plugin {
      ...
      if (visiting.has(id)) throw new RangeError(`Circular ${kind} preset: ${id}`);
      const input = source.get(id);
      if (!input) throw new RangeError(`Unknown ${kind} preset: ${id}`);
      visiting.add(id);
      const plugin = input.kind === 'world' ? defineWorld(input) : defineRunner(input);
      const base: Partial<WorldPlugin & Omit<RunnerPlugin, 'kind'>> = plugin.preset
