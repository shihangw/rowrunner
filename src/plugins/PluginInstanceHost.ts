...
export class PluginInstanceHost {
  ...
  draw(kind: Kind, definition: Definition, geometry: GeometryBuilder, frame: SceneFrame) {
    ...
    if (!instance) {
      ...
      if (typeof source !== 'object' || source === null || 'then' in source)
        throw new TypeError(`Plugin ${definition.id}: create must return a synchronous instance`);
      ...
      if (source.object !== undefined && source.object?.isObject3D !== true)
        throw new TypeError(`Plugin ${definition.id}: object must be a THREE.Object3D`);
      if (!draw && !source.object)
        throw new TypeError(`Plugin ${definition.id}: instance requires draw or object`);
    ...
    if (previous !== instance) {
      ...
      if (instance.source.object) this.roots[kind]?.add(instance.source.object);
