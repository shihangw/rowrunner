...
import { sceneCatalog, protagonistCatalog } from './SceneConfigurationCatalog.js';
export { CAMERA_MODES } from './CameraController.js';
export { CompletionCelebration } from './CompletionCelebration.js';
...
const crossProduct = (firstVector: readonly number[], secondVector: readonly number[]): Vec3 => [
...
const dotProduct = (firstVector: readonly number[], secondVector: readonly number[]) =>
  firstVector.reduce(
    (sum, component, componentIndex) => sum + component * secondVector[componentIndex],
    0,
  );
...
export class RoadScene {
  ...
  private pathEntry: { id: string; distance: number; time: number; introFinished: boolean } | null =
    null;
  ...
  constructor(
    ...
    const alias = <T>(oldValue: T | undefined, newValue: T | undefined, name: string) => {
    ...
    for (const entries of [scenes, protagonists])
      if (entries !== undefined) {
        if (!Array.isArray(entries)) throw new TypeError('scenes/protagonists must be arrays');
    ...
    const lineup = <T extends SceneDefinition | ProtagonistDefinition>(
      ...
      entries?.map((entry) =>
        typeof entry === 'object' && entry && 'kind' in entry ? entry.id : entry,
      );
    ...
    if (protagonist !== undefined && mascot !== undefined && protagonist !== mascot)
      throw new TypeError('protagonist and mascot must agree when both are provided');
    this.scenePreset = validatePreset(scene ?? this.scenes[0].id, this.scenes, 'scene');
    ...
    if (typeof autoBiomes !== 'boolean') throw new TypeError('autoBiomes must be a boolean');
  ...
  setCamera(mode: CameraMode) {
    ...
    if (mode === 'cinematic' && this.cameraMode !== mode) this.cinematic?.rebase(this.distance);
    if (mode !== this.cameraMode && this.pathEntry) this.pathEntry!.introFinished = true;
    this.cameraMode = mode;
  }
  setAutoBiomes(enabled: boolean) {
    if (typeof enabled !== 'boolean') throw new TypeError('autoBiomes must be a boolean');
  ...
  private projectPoint(localPoint: Vec3): Vec3 {
    const runnerTransformedPoint = this.transform ? this.transform(localPoint) : localPoint;
  ...
  private triangle(
    ...
    if (!unlit) {
      ...
      light = 0.65 + 0.35 * Math.abs(dotProduct(surfaceNormal, [0.3, 0.85, -0.4]));
    }
  ...
  private _gradientTriangle(
    ...
    const firstProjectedVertex = this.projectPoint(firstVertex),
      secondProjectedVertex = this.projectPoint(secondVertex),
      thirdProjectedVertex = this.projectPoint(thirdVertex);
    this.vertices.push(
  ...
  private glassTriangle(
    ...
    this.glass ??= [];
    for (const vertex of points) this.glass.push(...vertex, ...normal, ...color, opacity);
  }
  ...
  private mountain(x: number, z: number, width: number, height: number, color: Color) {
    const a: Vec3 = [x - width, -1, z - width],
      b: Vec3 = [x + width, -1, z - width],
      c: Vec3 = [x + width, -1, z + width],
      d: Vec3 = [x - width, -1, z + width],
      tip: Vec3 = [x, height, z];
    for (const [p, q] of [
      [a, b],
      [b, c],
  ...
  private ellipsoid(
    ...
    const face = (a: Vec3, b: Vec3, c: Vec3) => {
      if (!smooth) return this.triangle(a, b, c, color);
      ...
      for (const p of [a, b, c]) {
        const n = normalizedVector(p.map((v, i) => (v - center[i]) / (radius[i] * radius[i])));
        const light = 0.65 + 0.35 * Math.abs(dotProduct(n, [0.3, 0.85, -0.4]));
        const q = this.projectPoint(p);
        this.vertices.push(q[0], q[1], q[2], color[0] * light, color[1] * light, color[2] * light);
      }
    };
  ...
  private tube(a: Vec3, b: Vec3, radius: number, color: Color) {
    ...
    const u = normalizedVector(crossProduct(axis, Math.abs(axis[1]) > 0.9 ? [1, 0, 0] : [0, 1, 0]));
    const v = crossProduct(axis, u);
    const point = (p: Vec3, angle: number): Vec3 =>
      p.map(
        (value, i) => value + radius * (Math.cos(angle) * u[i] + Math.sin(angle) * v[i]),
      ) as unknown as Vec3;
    for (let i = 0; i < 8; i++) {
      const angle = (i / 8) * Math.PI * 2,
        next = ((i + 1) / 8) * Math.PI * 2;
      this.quad(point(a, angle), point(a, next), point(b, next), point(b, angle), color);
    }
  }
  private model(
    modelData: ModelData,
    ...
    for (const [a, b, c, index] of modelData.faces) {
      ...
      this.triangle(
        ...
        vertices[b],
        vertices[c],
        tint ? (tint.map((v) => v * (0.45 + brightness * 0.55)) as unknown as Color) : color,
      );
    }
  }
  private updateBiome(cinematicWorldChangeRequested = false, allowWorldChange = true) {
    if (
      this.autoBiomes &&
      allowWorldChange &&
    ...
    const paletteFor = (id: string) => this.scenes.find((p) => p.id === id)!.palette;
    ...
    if (this.biomeTransition) {
      ...
      palette = Object.fromEntries(
        Object.entries(paletteFor(from)).map(([key, a]) => [
          key,
          a.map((v: number, i: number) => v + (paletteFor(to)[key as keyof Palette][i] - v) * mix),
        ]),
  ...
  render(
    ...
    deltaSeconds: number,
    { reduced = false, completed = false }: { reduced?: boolean; completed?: boolean } = {},
  ) {
    ...
    const cinematicDirector = activeCameraMode === 'cinematic' ? this.cinematic : null;
    const isMotionReduced = reduced || this.reducedMotion.matches;
    const frameDeltaSeconds = Math.min(deltaSeconds, 0.05);
    const visualSpeed = isMotionReduced ? 0 : travelSpeed(processingRate, this.speedScale);
    let cinematicWorldChangeRequested = false;
    this.worldElapsedSeconds ??= 0;
    if (!isMotionReduced && !completed && processingRate > 0 && this.autoBiomes)
      this.worldElapsedSeconds += frameDeltaSeconds;
    if (!isMotionReduced) {
      this.distance += visualSpeed * frameDeltaSeconds;
      this.time += frameDeltaSeconds;
      if (processingRate > 1) this.cameraTime += frameDeltaSeconds;
      const isFadingOutWorld =
        this.biomeTransition?.synchronizeCamera && this.scenePreset === this.biomeTransition.from;
      if (isFadingOutWorld)
        cinematicDirector?.pass?.advance(processingRate, 0, this.distance, visualSpeed, false);
      if ((!this.pathEntry || this.pathEntry!.introFinished) && !isFadingOutWorld) {
        ...
        if (shotFinished) {
          ...
          cinematicWorldChangeRequested =
            shouldCycleWorld && this.worldElapsedSeconds >= this.worldSwitchIntervalSeconds;
      ...
      if (activeCameraMode === 'stationary')
        this.stationaryPass.advance(processingRate, frameDeltaSeconds, this.distance, visualSpeed);
    }
    const previousWorld = this.scenePreset;
    // Freeze an in-flight fade while paused, completed, or reducing motion.
    if (this.biomeTransition && (completed || processingRate <= 0) && !isMotionReduced)
    ...
    if (previousWorld !== this.scenePreset && this.biomeTransition?.synchronizeCamera) {
    ...
    } else if (!hasCurvedPath) this.pathEntry = null;
    let isPlayingPathIntroduction = false;
    if (hasCurvedPath && !this.pathEntry!.introFinished) {
      isPlayingPathIntroduction =
        !completed && this.time - this.pathEntry!.time < (world.path!.introDuration ?? 0);
    ...
    const worldOriginDistance = stationaryFrame ? stationaryFrame.worldDistance : this.distance,
      roadCenterXAt = hasCurvedPath
        ? () => 0
        : (z: number) =>
    ...
    const viewportWidth = Math.round(canvasBounds.width * pixelRatio),
      viewportHeight = Math.round(canvasBounds.height * pixelRatio);
    if (!viewportWidth || !viewportHeight) return;
    let fieldOfViewRadians =
      ((isStationary ? 58 : isMotionReduced ? 54 : 54 + 5 * Math.min(processingRate / 1500, 1)) *
        Math.PI) /
      180;
    if (isPlayingPathIntroduction)
      fieldOfViewRadians = Math.max(
        ...
        2 *
          Math.atan(
            Math.tan((((world.path!.introHorizontalFov ?? 70) / 2) * Math.PI) / 180) /
    ...
    if (world.path?.camera) pose = world.path!.camera(pose, isStationary, completed);
    if (isPlayingPathIntroduction) pose = { ...pose, label: `${world.name} · arrival` };
    ...
    const projectionScale = 1 / Math.tan(fieldOfViewRadians / 2),
      nearClipDistance = 0.7,
      farClipDistance = Math.max(1300, (stationaryFrame?.segmentLength ?? 0) * 2);
    const projectionMatrix = new Float32Array([
      ...
      0,
      0,
      (farClipDistance + nearClipDistance) / (nearClipDistance - farClipDistance),
      -1,
      0,
      0,
      (2 * farClipDistance * nearClipDistance) / (nearClipDistance - farClipDistance),
      0,
    ]);
    ...
    const runnerDefinition = this.protagonists.find((p) => p.id === this.protagonist)!;
    ...
    const runnerHeadingRadians =
      Math.atan2(roadCenterXAt(runnerZ + 0.1) - roadCenterXAt(runnerZ - 0.1), 0.2) +
      (runnerDefinition.forwardAxis === '-z' ? Math.PI : 0);
    const headingCosine = Math.cos(runnerHeadingRadians),
      headingSine = Math.sin(runnerHeadingRadians);
    const frame: SceneFrame = Object.freeze({
      ...
      far: roadEnd,
      roadLight: (z: number) =>
        roadSurface(palette, worldOriginDistance + z, renewalFrontDistance, this.roadEffect).accent,
      projectPoint: orbit?.localPoint ?? ((point: Vec3): Vec3 => [...point]),
      view: stationaryFrame
        ? Object.freeze({ ...pose, clearance: 24 + stationaryFrame.segmentLength * 0.1 })
        : null,
    });
    ...
    if (world.road) {
      ...
      if (orbit) {
        ...
        drawRoad(
          {
            quad: (firstVertex, secondVertex, thirdVertex, fourthVertex, color, unlit) =>
              this.quad(
      ...
      } else drawRoad(this.geometry, roadOptions);
      ...
      for (let ring = 0; ring < 10; ring++) {
        const inner = ring / 10,
          outer = (ring + 1) / 10;
        const color = shadowBase.map((v) => v * (0.53 + 0.4175 * outer)) as unknown as Color;
        const point = (angle: number, r: number): Vec3 => [
          runnerX + runnerDefinition.offset[0] + Math.cos(angle) * r * 1.8 * runnerDefinition.scale,
          0.032,
          runnerZ +
    ...
    this.sceneRenderer.draw(
      ...
      palette,
      fade,
      Math.max(world.path?.fogDistance ?? 360, (stationaryFrame?.segmentLength ?? 0) * 1.2),
      this.particles,
      this.glass,
    );
  }
