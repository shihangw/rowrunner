import type {Palette, SkyDefinition, SkyFrame, Vec3} from './scene_types.js';
type AtmosphereCameraState = {view: Float32Array; eye: Vec3; fov: number};
import * as THREE from 'three';
import * as shaders from './scene_shader_programs.js';
...
class DynamicGeometryMesh extends THREE.Mesh<
  THREE.BufferGeometry,
  THREE.RawShaderMaterial
> {
  private readonly vertexAttributeLayout: readonly (readonly [
    string,
    number,
  ])[];
  ...
  constructor(
    ...
    this.vertexStride = vertexAttributeLayout.reduce(
      (n, [, size]) => n + size,
      0,
    );
    ...
    if (!this.vertexBuffer || values.length > this.vertexBuffer.array.length) {
      ...
      const capacity =
        Math.ceil(
          Math.max(
            values.length,
            (this.vertexBuffer?.array.length ?? 0) * 1.5,
          ) / this.vertexStride,
        ) * this.vertexStride;
...
export class SceneRenderer {
  ...
  constructor(canvas: HTMLCanvasElement) {
    ...
    this.geometryMaterial = createShaderMaterial(
      shaders.geometryVertex,
      shaders.geometryFragment,
    );
    ...
    this.postProcessingMaterial = createShaderMaterial(
      shaders.postVertex,
      shaders.postFragment,
      {
        depthTest: false,
        depthWrite: false,
      },
  ...
  begin(
    ...
    if (
      ...
      this.backdropTexture = new THREE.FramebufferTexture(
        viewportWidth,
        viewportHeight,
      );
    }
  ...
  draw(
    ...
    if (!(this.scene.fog instanceof THREE.FogExp2)) {
      this.scene.fog = new THREE.FogExp2(0);
    }
  ...
  dispose() {
    ...
    for (const sky of this.skyMaterials.values()) {
      sky.dispose();
    }
