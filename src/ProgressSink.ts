import type {
  ...
  SinkOptions,
  SmootherOptions,
} from './ProgressDataTypes.js';
export type * from './ProgressDataTypes.js';
export * from './ProgressDataSchemas.js';
import {
  ProgressSampleSchema,
  SinkOptionsSchema,
  SmootherOptionsSchema,
} from './ProgressDataSchemas.js';
...
export class ProgressSink {
  ...
  push(progressSample: ProgressSample, receivedAt = Date.now()): PushResult {
    if (!Number.isFinite(receivedAt)) throw new TypeError('Invalid receipt time');
    ...
    if (status === 'running' && targetTotal !== null && completed >= targetTotal)
      status = 'completed';
    ...
    if (previous) {
      if (timestamp < previous.timestamp) return { accepted: false, reason: 'out-of-order' };
      if (timestamp === previous.timestamp) {
        const duplicate = (Object.keys(sample) as (keyof NormalizedSample)[]).every(
          (key) => sample[key] === previous[key],
        );
        return { accepted: false, reason: duplicate ? 'duplicate' : 'timestamp-conflict' };
      }
    ...
    const didResume = previous && previous.status !== 'running' && status === 'running';
    const hasLongSampleGap = previous && timestamp - previous.timestamp > this.staleAfterMs;
    if (!previous || didCounterReset || didResume || hasLongSampleGap) {
      this.samples = [];
      this.lastAdvance = timestamp;
      this.resetCount = previous ? this.resetCount + Number(didCounterReset) : 0;
    }
    if (previous && completed > previous.completed) this.lastAdvance = timestamp;
    ...
    while (this.samples.length > 2 && this.samples[1].timestamp <= windowStartTimestamp)
      this.samples.shift();
    // Bound memory even if a producer sends sub-millisecond timestamps.
    if (this.samples.length > 2048) this.samples.splice(1, this.samples.length - 2048);
    this._latest = sample;
    this._receivedAt = receivedAt;
    return {
      accepted: true,
      reason: didCounterReset ? 'counter-reset' : !previous ? 'new-run' : 'update',
      sample,
    };
  }
  ...
  snapshot(currentTimestamp = Date.now()): ProgressSnapshot {
    ...
    if (state === 'running') {
      if (ageMs >= this.staleAfterMs) state = 'stale';
      else if (rate === null) state = 'warming';
      else if (latestSample.timestamp - this.lastAdvance! >= this.idleAfterMs) state = 'stalled';
    }
      ...
      etaSeconds:
        state === 'completed'
          ...
          : latestSample.targetTotal !== null && targetRate > 0
            ? Math.max(0, latestSample.targetTotal - latestSample.completed) / targetRate
            : null,
    };
  }
}
...
export class RateSmoother {
  ...
  constructor(options: SmootherOptions = {}) {
    const { riseSeconds, fallSeconds } = parseInput(SmootherOptionsSchema, options);
  ...
  step(target: number, deltaSeconds: number) {
    if (
      ...
      return this.value;
    const timeConstantSeconds = target > this.value ? this.riseSeconds : this.fallSeconds;
    this._value += (target - this.value) * -Math.expm1(-deltaSeconds / timeConstantSeconds);
    if (target === 0 && this.value < 0.01) this._value = 0;
    return this.value;
  }
  reset() {
    this._value = 0;
