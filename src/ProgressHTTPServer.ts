...
import type { SinkOptions } from './ProgressDataTypes.js';
...
import { ProgressSink, ProgressSampleSchema } from './ProgressSink.js';
...
export function createProgressServer({
  ...
  const sendJSONResponse = (response: ServerResponse, status: number, data: unknown) => {
  ...
  const server = http.createServer(async (request, response) => {
    try {
      ...
      const route = /^\/api\/runs\/([a-zA-Z0-9_-]{1,80})\/(progress|events)$/.exec(url.pathname);
      if (route) {
        const [, runId, resource] = route;
        if (resource === 'events' && request.method === 'GET') {
          if ([...eventListenersByRunID.values()].reduce((sum, set) => sum + set.size, 0) >= 100)
            return sendJSONResponse(response, 503, { error: 'Too many listeners' });
          ...
          if (!eventListenersByRunID.has(runId)) eventListenersByRunID.set(runId, new Set());
          ...
          request.on('close', () => {
            ...
            if (!eventListenersByRunID.get(runId)?.size) eventListenersByRunID.delete(runId);
          });
          return;
        }
        ...
        if (resource === 'progress' && request.method === 'POST') {
          if (!request.headers['content-type']?.startsWith('application/json'))
            return sendJSONResponse(response, 415, { error: 'Use application/json' });
          let body = '';
          let bodyByteCount = 0;
          ...
          if (
            ...
            return sendJSONResponse(response, 400, { error: 'Expected a progress object' });
          if (
            'runId' in requestPayload &&
            requestPayload.runId !== undefined &&
            requestPayload.runId !== runId
          )
            return sendJSONResponse(response, 400, { error: 'runId conflicts with URL' });
          if (!progressSinksByRunID.has(runId) && progressSinksByRunID.size >= 100)
            return sendJSONResponse(response, 409, {
              error: 'Run limit reached; restart the local server',
            });
          const progressSink = progressSinksByRunID.get(runId) ?? new ProgressSink(sinkOptions);
