import { z } from 'zod';
import type { DrawFunction, PluginFactory, SkyFrame, PathFrame, CameraPose } from './SceneTypes.js';
...
export const PaletteSchema = z
  ...
  .readonly();
export const PaletteOverridesSchema = PaletteSchema.unwrap().partial().readonly();
export const DefinitionIdSchema = z.string().regex(/^[a-zA-Z0-9][a-zA-Z0-9_-]*$/);
...
const callback = <T>(name: string) =>
  z.custom<T>((value) => typeof value === 'function', `${name} must be a function`);
export const SkyDefinitionSchema = z
  .object({
    /** GLSL fragment shader, using the fullscreen uv varying. */
    fragment: z.string(),
    uniforms:
      callback<(frame: SkyFrame) => Record<string, number | readonly number[] | Texture>>(
        'uniforms',
      ).optional(),
  })
  .readonly();
export const WorldPathSchema = z
  .object({
    createFrame: callback<(distance: number, entryDistance: number) => PathFrame>('createFrame'),
    camera:
      callback<(pose: CameraPose, stationary: boolean, completed: boolean) => CameraPose>(
        'camera',
      ).optional(),
