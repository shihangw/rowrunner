...
import { RoadScene, SCENE_PRESETS, MASCOT_PRESETS } from './fixtures/ExampleContentFixtures.js';
...
test('mascot meshes contain only finite vertices and colors, including at rest and at high speed', () => {
  for (const preset of MASCOT_PRESETS)
    for (const rate of [0, 500, 1_000_000]) {
      ...
      assert.equal(mesh.vertices.length % 18, 0, `${preset.id}: incomplete triangles`);
      assert.ok(mesh.vertices.every(Number.isFinite), `${preset.id}: nonfinite geometry`);
...
test('cinematic visits all ten views once per shuffled cycle, with fixed full stationary passes', () => {
  ...
  for (let visit = 0; visit < 30; visit++) {
    ...
    while (director.shot === shot && elapsed < 30) {
      ...
      if (director.shot === shot && director.pass) {
        ...
        if (fixed) assert.deepEqual(pose, fixed);
        else fixed = pose;
        assert.ok(
          Math.abs(director.pass.worldDistance + director.pass.frame.protagonistZ - distance) <
            1e-8,
        );
      }
    }
  ...
  for (let cycle = 0; cycle < 3; cycle++)
    assert.deepEqual(new Set(seen.slice(cycle * 10, cycle * 10 + 10)), new Set(CINEMATIC_SHOTS));
  ...
  for (const from of ['chase', 'side', 'starboard', 'aerial', 'approach'])
    for (const to of ['chase', 'side', 'starboard', 'aerial', 'approach']) {
      ...
      for (let i = 0; i <= 400; i++) {
        ...
        assert.ok(Math.hypot(pose.eye[0] - pose.target[0], pose.eye[2] - pose.target[2]) >= 10);
...
test('custom catalogs copy caller data and isolate scene instances', () => {
  ...
  assert.notDeepEqual(sceneCatalog()[0].palette.accent, worlds[0].palette.accent);
  const offset = [1, 3, -2];
...
test('invalid custom definitions fail before acquiring browser resources', () => {
  for (const scenes of [
    ...
    assert.throws(() => new RoadScene(null, { scenes }), /scenes|palette|accent|road/);
  }
  for (const protagonists of [
    ...
    assert.throws(() => new RoadScene(null, { protagonists }), /protagonists|scale|offset|roll/);
  }
  assert.throws(
    () => new RoadScene(null, { scenes: [{ id: 'glacier', palette: { accent: Array(3) } }] }),
    TypeError,
  );
  assert.throws(
  ...
  assert.throws(() => new RoadScene(null, { scenes: ['glacier'], scene: 'clouds' }), RangeError);
});
...
test('stationary road segments preserve the same world speed and take at least six seconds', () => {
  for (const rate of [80, 500, 1800, 1e9]) {
    ...
    while (pass.startDistance === 500 && elapsed < 40) {
      ...
      assert.ok(Math.abs(frame.worldDistance + frame.protagonistZ - distance) < 1e-6);
      if (pass.startDistance === 500)
        assert.ok(Math.abs(frame.protagonistZ - previousZ - travelSpeed(rate) * 0.05) < 1e-6);
    }
...
test('stationary speed spikes do not slow travel or move the camera before the minimum shot time', () => {
  ...
  const fixedOrigin = pass.worldDistance,
    fixedLength = pass.segmentLength;
  for (let i = 0; i < 79; i++) {
    const previous = pass.frame.protagonistZ;
    distance += travelSpeed(1800) * 0.05;
    pass.advance(1800, 0.05, distance);
    assert.ok(Math.abs(pass.frame.protagonistZ - previous - travelSpeed(1800) * 0.05) < 1e-8);
...
test('stationary cuts cover five distinct angles without changing a shot in progress', () => {
  ...
  for (let cut = 0; cut < 15; cut++) {
    ...
    if (cut < 5) firstDeck.add(shot);
    const pose = stationaryPose(pass.frame, centerX);
    assert.ok(pose.eye.every(Number.isFinite));
    assert.ok(pose.eye[1] >= 23);
    if (shot === 'approach') assert.ok(pose.eye[2] > pose.target[2]);
    if (shot === 'departure' || shot === 'diagonal') assert.ok(pose.eye[2] < pose.target[2]);
    if (shot === 'crossing') assert.equal(pose.eye[2], pose.target[2]);
    const horizontal = Math.hypot(pose.eye[0] - pose.target[0], pose.eye[2] - pose.target[2]);
    assert.ok(
...
for (const [rate, intervalSeconds] of [
  ...
  test(`scheduled worlds fade with cinematic shots at ${intervalSeconds}s and rate ${rate}`, () => {
    ...
    try {
      ...
      const scene = Object.assign(Object.create(RoadScene.prototype), {
        scenes: sceneCatalog(worldIDs.map((id) => ({ id, road: false, draw() {} }))),
      ...
      for (let frame = 0; frame < 6000 && worldChanges < 6; frame++) {
        ...
        if (transition && scene.sceneRenderer.fade > 0 && scene.sceneRenderer.fade < 1) {
          if (scene.world === transition.from) sawFadeOut = true;
          else sawFadeIn = true;
        }
        if (cameraChanged) {
          cameraChanges++;
      ...
      scene.setAutoBiomes(false);
      for (let frame = 0; frame < 400; frame++) scene.render(rate, 0.05);
