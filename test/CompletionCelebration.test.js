...
import { CompletionCelebration } from './fixtures/ExampleContentFixtures.js';
...
const done = { runId: 'migration', state: 'completed', completed: 1200, unit: 'rows' };
...
test('only completed snapshots celebrate; target completion and explicit completion both work', () => {
  const c = effect();
  for (const state of ['waiting', 'warming', 'running', 'paused', 'stalled', 'stale', 'failed']) {
    c.update({ ...done, state }, { now: 0 });
    assert.equal(c.element.hidden, true, state);
  }
  const sink = new ProgressSink();
  sink.push({ runId: 'target', completed: 100, targetTotal: 100, timestamp: 1000 }, 1000);
  c.update(sink.snapshot(1000), { now: 0 });
  assert.equal(c.element.hidden, false);
  c.update({ state: 'waiting' }, { now: 10 });
  assert.equal(c.element.hidden, true);
  sink.push({ runId: 'open-ended', completed: 50, status: 'completed', timestamp: 2000 }, 2000);
