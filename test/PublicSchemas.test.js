...
import { sceneCatalog, protagonistCatalog } from '../src/SceneConfigurationCatalog.js';
...
test('public progress schemas normalize timestamps without inventing stateful defaults', () => {
  ...
  assert.equal(ProgressSampleSchema.parse({ ...sample, targetTotal: null }).targetTotal, null);
  ...
  for (const invalid of [
    ...
    assert.equal(ProgressSampleSchema.safeParse({ ...sample, ...invalid }).success, false);
});
...
test('sink retains state-dependent validation and exposes structured schema errors atomically', () => {
  ...
  const result = sink.push({ ...sample, targetTotal: 2000, label: 'Migration' }, timestamp);
  ...
  const future = ProgressSampleSchema.parse({ ...sample, timestamp: timestamp + 6000 });
  assert.throws(() => sink.push(future, timestamp), /future/);
  sink.push({ ...sample, completed: 1000, timestamp: timestamp + 1000 }, timestamp + 1000);
...
test('definition schemas preserve callback identity, clone values and validate both registration paths', () => {
  ...
  const runner = defineRunner({ id: 'runner', draw, scale: 2, offset: [1, 2, 3] });
  const options = SceneOptionsSchema.parse({ worlds: [world], runners: [runner] });
  ...
  for (const bad of [
    ...
    assert.throws(() => protagonistCatalog([{ id: 'bad', draw, ...bad }]), TypeError);
  }
  for (const bad of [
  ...
  assert.equal(SceneOptionsSchema.safeParse({ camera: 'overhead' }).success, false);
...
test('explicit undefined overrides still inherit preset values', () => {
  const draw = () => {};
  const base = defineWorld({ id: 'base', draw, road: false, palette: { accent: [0.1, 0.2, 0.3] } });
  const world = sceneCatalog(
    [{ id: 'child', preset: 'base', road: undefined, palette: { accent: undefined } }],
    [base],
  )[0];
