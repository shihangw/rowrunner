...
import bookie from '../examples/src/runners/bookie/BookieRunner.ts';
import glacier from '../examples/src/worlds/ion-glacier/IonGlacierWorld.ts';
import { createPluginRegistry } from '../src/plugins/PluginRegistry.js';
import { PluginInstanceHost } from '../src/plugins/PluginInstanceHost.js';
import { sceneCatalog, protagonistCatalog } from './fixtures/ExampleContentFixtures.js';
import { RoadScene } from '../src/RoadScene.js';
...
test('plugin catalogs inherit presets and remain isolated from input and other instances', () => {
  ...
  const runner = defineRunner({ id: 'little', preset: 'bookie', scale: 0.5, offset });
  const registry = createPluginRegistry([glacier, bookie, world, runner]);
  accent[0] = 1;
  offset[1] = 20;
  const worlds = sceneCatalog(['blue'], registry.worlds),
    runners = protagonistCatalog(['little'], registry.runners);
...
test('invalid registrations fail before WebGL initialization', () => {
  ...
  for (const plugins of [
    ...
    assert.throws(() => new RoadScene(null, { plugins }), /Duplicate|version|declare|preset/);
  }
  for (const palette of [{ unknown: [0, 0, 0] }, { accent: [2, 0, 0] }, { accent: Array(3) }, []])
    assert.throws(() => defineWorld({ id: 'bad', draw() {}, palette }), TypeError);
  assert.throws(() => defineRunner({ id: 'bad', draw() {}, offset: 12 }), /offset/);
  assert.throws(() => new RoadScene(null, { world: 'glacier', scene: 'matrix' }), /aliases/);
  assert.throws(() => new RoadScene(null, { runner: 'bookie', protagonist: 'courier' }), /aliases/);
  assert.throws(() => new RoadScene(null, { worlds: ['unknown'], runners: [bookie] }), RangeError);
});
...
test('stateful plugins enter, update, exit, re-enter and dispose once per scene', () => {
  ...
  const other = defineRunner({ id: 'other', draw() {} }),
    a = new PluginInstanceHost(),
    b = new PluginInstanceHost(),
    mesh = [];
...
test('world and runner are active independently and all resources release after hook errors', () => {
  ...
  assert.deepEqual(events, ['world exit', 'runner exit', 'world dispose', 'runner dispose']);
  host.dispose();
});
...
test('factory replaces preset geometry and rejects invalid synchronous instances', () => {
  ...
  for (const value of [undefined, null, 3, Promise.resolve({}), { update: true }, { draw: true }]) {
    assert.throws(
      () =>
...
test('native Three.js plugins mount, switch, animate and release exclusively owned resources', async () => {
  ...
  const roots = { world: new Group(), runner: new Group() },
    host = new PluginInstanceHost(roots);
  ...
  const other = defineRunner({ id: 'other-native', create: () => ({ object: new Group() }) });
  ...
  host.dispose();
  host.dispose();
  assert.deepEqual(new Set(disposed), new Set(['geometry', 'material', 'texture']));
...
test('applications supply all content and inheritance resolves independently of array order', () => {
  const base = defineWorld({ id: 'base', draw() {}, palette: { accent: [0.2, 0.3, 0.4] } });
  const variant = defineWorld({ id: 'variant', preset: 'base', palette: { sky: [0, 0, 0] } });
