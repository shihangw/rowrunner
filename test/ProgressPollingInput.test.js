...
import { startProgressPolling } from '../examples/src/inputs/ProgressPollingInput.ts';
import { readSolanaTransactionProgress } from '../examples/src/inputs/SolanaTransactionSource.ts';
...
test('timeouts and malformed samples retry without publishing synthetic progress', async (t) => {
  ...
  const stop = startProgressPolling({
    readSample(signal) {
      ...
      return Promise.resolve(requestCount === 2 ? { ...sample(), completed: 'invalid' } : sample());
    },
    onSample(value) {
      received.push(value);
    },
...
test('Solana adapter reads finalized totals and rejects failed or malformed RPC responses', async () => {
  ...
  const progress = await readSolanaTransactionProgress(signal, async (url, options) => {
  ...
  for (const response of [
    ...
    await assert.rejects(readSolanaTransactionProgress(signal, async () => response));
});
