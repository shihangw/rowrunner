...
import { drawExhaust } from '../examples/src/runners/shared/ExhaustParticles.js';
...
test('exhaust is bounded, deterministic and fades over particle lifetimes', () => {
  const collect = (time, rate, still = false) => {
    const particles = [];
    drawExhaust({ particle: (...p) => particles.push(p) }, time, rate, still, [1, 0, 1], 1);
    return particles;
  };
    ...
    for (const preset of ['bookie'])
      for (const mode of CAMERA_MODES) {
        const scene = Object.assign(Object.create(RoadScene.prototype), {
          ...
          protagonists: protagonistCatalog([{ id: preset, scale: 2, bob: 0, roll: 0 }]),
          scenePreset: 'empty',
          mascotPreset: preset,
          cameraMode: mode,
          distance: 0,
        ...
        for (let i = 0; i < scene.particles.length; i += 10) {
          assert.ok(scene.particles[i + 2] < heroZ, `${preset} exhaust points forward in ${mode}`);
