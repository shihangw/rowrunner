...
import { ProgressSink } from '../src/ProgressSink.js';
import { ProgressHistory } from '../examples/src/ProgressHistory.ts';
...
test('HUD chart records cumulative source counts, not rate or animation frames', () => {
  ...
  const push = (completed, timestamp, extra = {}) =>
    history.add(sink.push({ runId: 'chart', completed, timestamp, ...extra }, 100000));
  ...
  const plottedHeights = [...chart.line.matchAll(/[ML][\d.]+,([\d.]+)/g)].map((match) =>
    Number(match[1]),
  );
...
test('large cumulative totals use recent growth for readable bounds and headroom', () => {
  ...
  for (let index = 0; index < 3; index++) {
    history.add({
      accepted: true,
      reason: 'update',
      sample: { timestamp: index * 10000, completed: initialCount + index * 50000 },
    });
  }
  const chart = history.plot();
...
test('rolling window clips older segments without mutating confirmed samples', () => {
  ...
  for (const [timestamp, completed] of [
    ...
    history.add({ accepted: true, reason: 'update', sample: { timestamp, completed } });
  }
  ...
  const expectedBoundaryHeight = 68 - ((2000 - chart.min) / (chart.max - chart.min)) * 64;
  assert.ok(chart.line.startsWith(`M2.00,${expectedBoundaryHeight.toFixed(2)}`));
...
test('single samples, flat counters and zero counters keep finite nondegenerate axes', () => {
  for (const completed of [0, 5, 552190000000]) {
    ...
    for (const timestamp of [0, 10000, 70000]) {
      ...
      if (completed > 1000) assert.ok(chart.min > 0);
    }
  }
});
