...
test('HTTP sink derives rates, isolates runs, and reports rejected updates', async (t) => {
  ...
  const snapshot = await (await fetch(`${root}/api/runs/migration/progress`)).json();
  ...
  assert.equal((await post({ timestamp: timestamp - 1000, completed: 0 })).status, 409);
  assert.equal((await post({ timestamp, completed: 50 }, 'second')).status, 200);
  assert.equal((await (await fetch(`${root}/api/runs/second/progress`)).json()).completed, 50);
  assert.equal((await (await fetch(`${root}/api/runs/unknown/progress`)).json()).state, 'waiting');
});

test('HTTP input validation, body limits, and static file allowlist', async (t) => {
  const { root, post } = await fixture(t);
  const good = { timestamp: Date.now(), completed: 0 };
  for (const input of [null, [], { ...good, completed: -1 }, { ...good, runId: 'wrong' }])
    assert.equal((await post(input)).status, 400);
  assert.equal((await post(good, 'migration', { Origin: 'https://other.example' })).status, 403);
  assert.equal((await post(good, 'migration', { 'Content-Type': 'text/plain' })).status, 415);
  ...
  assert.equal(
    (await fetch(`${root}/api/runs/migration/progress`, { method: 'DELETE' })).status,
    405,
  );
});
...
test('SSE sends current sample and broadcasts accepted progress', async (t) => {
  ...
  const response = await fetch(`${root}/api/runs/migration/events`, { signal: abort.signal });
