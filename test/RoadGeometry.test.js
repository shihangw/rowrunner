...
import { roadSurface, drawRoad } from '../src/RoadGeometry.js';
import { sceneCatalog, protagonistCatalog } from './fixtures/ExampleContentFixtures.js';
import { StationaryCameraPass, travelSpeed } from '../src/CameraController.js';
    ...
    drawRoad(
      {
        quad(a, b, c, d, color) {
          if (a[1] === 0 && b[1] === 0 && a[0] === -7) {
            ...
            if (worldZ >= 100 && worldZ < 200) panels.push({ worldZ, color });
          }
        },
      },
      { distance, front: 150, centerX: () => 0, palette, far: 660, effect: 'renewal' },
    );
    return panels;
  };
  ...
  try {
    for (const mode of ['stationary', 'chase']) {
      const scene = Object.assign(Object.create(RoadScene.prototype), {
        ...
        protagonists: protagonistCatalog([{ id: 'orb', offset: [0, 2, 9], draw() {} }]),
        scenePreset: 'empty',
        mascotPreset: 'orb',
        cameraMode: mode,
        distance: 100,
      ...
      const checkFront = () => {
        scene.render(0, 0);
        const origin =
          mode === 'stationary' ? scene.stationaryPass.frame.worldDistance : scene.distance;
        // The sweep's leading white line sits at .045 above the deck.
        const points = [];
        for (let i = 0; i < scene.vertices.length; i += 6)
          if (scene.vertices[i + 1] === 0.045) points.push(scene.vertices[i + 2] + origin);
        assert.ok(points.length > 0);
        assert.ok(Math.abs(Math.max(...points) - (scene.distance + 9 + 0.03)) < 1e-8);
        assert.ok(scene.vertices.every(Number.isFinite));
      };
      checkFront();
      for (let i = 0; i < 60; i++) scene.render(500, 1 / 60);
