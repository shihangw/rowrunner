...
import {drawRoad} from '../src/road_geometry.js';
import {drawWorld, PALETTES} from './fixtures/example_content_fixtures.js';
    ...
    for (const side of [-1, 1]) {
      for (const rate of [80, 500, 1800]) {
        const frame = {
          shot,
          side,
          segmentLength: Math.max(180, travelSpeed(rate) * 6),
        };
        const pose = stationaryPose(frame, () => 0);
        const normal = sceneRange(
          pose,
          (58 * Math.PI) / 180,
          1.6,
          frame.segmentLength,
        );
        const wide = sceneRange(
          pose,
          (58 * Math.PI) / 180,
          4,
          frame.segmentLength,
...
test('road panels, stripes, and every biome extend behind the starting position', () => {
  ...
  for (const distance of [0, 2500]) {
    ...
    let maxPanel = -Infinity;
    let minStripe = Infinity;
    drawRoad(
      {
        quad(a, b, c) {
          if (a[1] === 0 && b[1] === 0) {
          ...
          if (a[1] === 0.06) {
            minStripe = Math.min(minStripe, a[2]);
          }
        },
      },
      {
    ...
    for (const [id, palette] of Object.entries(PALETTES)) {
      ...
      assert.ok(
        behind > 0,
        `${id} has no scenery behind the start at distance ${distance}`,
      );
    }
  }
});
