...
import {
  ...
  ORBIT_RADIUS,
  ORBIT_TRAVEL_SCALE,
} from '../examples/src/worlds/binary-eclipse/BinaryEclipseOrbit.js';
...
test('binary and road share one center and a closed orbit at astronomical scale', () => {
  ...
  for (const distance of [0, 100, 10000, LOCAL_ORBIT_LENGTH * 20]) {
    ...
    assert.ok(separation(point, orbitPoint([0, 0, LOCAL_ORBIT_LENGTH], distance)) < 1e-7);
  }
});
...
test('road panels stay fixed on the orbit across tracking and stationary origins', () => {
  for (const origin of [0, 55, 390, 10000]) {
    ...
    assert.ok(
      Math.abs(separation(frame.localPoint([-7, 0, 0]), frame.localPoint([7, 0, 0])) - 14) < 1e-8,
    );
    assert.ok(
      Math.abs(separation(frame.localPoint([0, 0, 0]), frame.localPoint([0, 0, 0.001])) - 0.001) <
        1e-8,
    );
  }
...
test('generated road curves around the binary center and retains painted stripes', () => {
  for (const distance of [0, 3600]) {
    ...
    const center = BINARY_CENTER.map((v, i) => (v - orbit.origin[i]) / ORBIT_TRAVEL_SCALE);
    let panels = 0,
      stripes = 0;
    drawBinaryRoad(
      {
        quad(a, b, c, d) {
          ...
          if (points.every((p) => Math.abs(p[1]) < 1e-8)) {
            ...
            for (const p of points) {
              ...
              assert.ok(Math.abs(Math.abs(radial - ORBIT_RADIUS / ORBIT_TRAVEL_SCALE) - 7) < 1e-7);
            }
          }
          if (points.every((p) => Math.abs(p[1] - 0.06) < 1e-8)) stripes++;
        },
      },
      {
        distance,
...
test('orbital viewpoint changes visibly at normal rates', () => {
  ...
  const angle = Math.acos((a[0] * b[0] + a[2] * b[2]) / (ORBIT_RADIUS * ORBIT_RADIUS));
...
test('entering the biome resets orbital phase without resetting travel distance', () => {
  ...
  for (const distance of [0, 7000, 55000]) {
    ...
    assert.deepEqual(arrival.worldPoint([32, 11, -3]), start.worldPoint([32, 11, -3]));
    const later = createBinaryOrbit(distance + 250, distance);
    assert.ok(separation(later.origin, arrival.origin) > 1000);
    assert.ok(separation(arrival.worldPoint([0, 0, 250]), later.worldPoint([0, 0, 0])) < 1e-7);
  }
});
