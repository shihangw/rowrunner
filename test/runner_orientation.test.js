    ...
    for (const id of ['bookie', 'courier', 'paper-plane', 'custom']) {
      ...
      for (const mode of CAMERA_MODES) {
        for (const distance of [0, 250, 650]) {
          ...
          const slope =
            0.1 * Math.cos((distance + z) / 240) +
            0.12 * Math.cos((distance + z) / 100);
...
test('model forward axes and propulsion wakes agree', () => {
  ...
  assert.equal(
    protagonistCatalog([{id: 'robot', forwardAxis: '+z', draw() {}}])[0]
      .forwardAxis,
    '+z',
  );
  assert.throws(
    () => protagonistCatalog([{id: 'bookie', forwardAxis: 'sideways'}]),
    TypeError,
  );
  ...
  for (let i = 2; i < m.particles.length; i += 10) {
    assert.ok(m.particles[i] > 1);
  }
});
