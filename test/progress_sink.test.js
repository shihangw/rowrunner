...
const push = (sink, ms, completed, extra = {}) =>
  sink.push(
    {runId: 'test', timestamp: base + ms, completed, ...extra},
    base + ms,
  );
...
test('duplicates, timestamp conflicts, and out-of-order samples never mutate freshness', () => {
  ...
  assert.equal(
    sink.push(
      {runId: 'test', timestamp: base + 2000, completed: 1000},
      base + 20000,
    ).reason,
    'duplicate',
  );
...
test('old measurements received now are stale and produce no ETA or animation', () => {
  ...
  sink.push(
    {runId: 'test', completed: 1000, timestamp: base + 2000},
    base + 100000,
  );
...
test('explicit completion and failure are terminal; new run clears old metadata', () => {
  for (const status of ['completed', 'failed']) {
    ...
    sink.push(
      {runId: 'new', completed: 0, timestamp: base + 50000},
      base + 50000,
    );
...
test('invalid samples fail atomically and ISO timezone timestamps are normalized', () => {
  ...
  for (const changes of [
    ...
    assert.throws(
      () =>
        sink.push(
          {runId: 'test', completed: 200, timestamp: base + 1000, ...changes},
          base,
        ),
      TypeError,
    );
  ...
  assert.equal(
    sink.push({runId: 'test', completed: 200, timestamp}, base + 1000).sample
      .timestamp,
    base + 1000,
  );
